...
# Taken from https://github.com/mohaimenz/acdnet/blob/master/tf/resources/models.py
...
class ConvBlock:
    def __init__(self, filters, kernel_size, stride=(1,1), padding='valid', use_bias=False):
        self.conv = L.Conv2D(filters=filters, kernel_size=kernel_size, strides=stride, padding=padding, kernel_initializer=tf.initializers.he_normal(), use_bias=use_bias) 
...
class ACDNet:
    def __init__(self, input_length=66650, n_class=50, sr=44100, ch_conf=None):
        ...
        self.fcn = L.Dense(n_class, kernel_initializer=tf.initializers.he_normal()) 
...
#TODO define our version of TinyACDNet
def student_adcnet(input_shape, nb_classes):
    n_frames = (20480/1000)*10  #No of frames per 10ms
    sfeb_pool_size = 32 #int(n_frames/(2*2))
    
    input_layer = L.Input(shape=(1, input_shape[0], 1))

    #Spectral 
    sfeb = L.Conv2D(filters=8, kernel_size=(1, 3), strides=(1,2), 
                         kernel_initializer=tf.initializers.he_normal(), use_bias=False)(input_layer)
    #sfeb = L.BatchNormalization()(sfeb)
    sfeb = L.ReLU()(sfeb)

    sfeb = L.MaxPooling2D(pool_size=(1, 32))(sfeb)

    sfeb = L.Conv2D(filters=21, kernel_size=(1, 3), strides=(1,1), 
                         kernel_initializer=tf.initializers.he_normal(), use_bias=False)(sfeb) 
    #sfeb = L.BatchNormalization()(sfeb)
    sfeb = L.ReLU()(sfeb)

    sfeb = L.MaxPooling2D(pool_size=(1, 4), name="conv_sfeb")(sfeb)

    #Temporal 
    tfeb = L.Permute((3, 2, 1))(sfeb)

    tfeb = L.Conv2D(filters=16, kernel_size=(3, 3), padding='same',
                         kernel_initializer=tf.initializers.he_normal(), use_bias=False)(tfeb)

    #tfeb = L.BatchNormalization()(tfeb)
    tfeb = L.ReLU()(tfeb)
    tfeb = L.MaxPooling2D(pool_size=(2,2))(tfeb)
    ...
    #tfeb = L.BatchNormalization()(tfeb)
    tfeb = L.ReLU()(tfeb)
    tfeb = L.MaxPooling2D(pool_size=(2,4))(tfeb)

    output_main = L.Flatten()(tfeb)
    output_main = L.Dense(units=64, activation='relu')(output_main)
    output_main = L.Dense(units=nb_classes, activation='softmax', name="output_main")(output_main)
    
    model = Model(inputs=input_layer, outputs={'conv_sfeb': sfeb, 'output_main': output_main}, name="adc")
    return model